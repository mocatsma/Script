...
global OrbiterLog is "0:/Orbiter-x.txt".
// Display a message
...
//Uses Exists removes HasFile and Require Fx's
...
IF ADDONS:RT:HASCONNECTION(SHIP) and not inflight {
  IF EXISTS(afp) {
    ...
    Switch to 0.
    MOVEPATH(afp,"oOrbiter-x.update.ks").
    SWITCH TO 1.
    IF EXISTS("1:/update.ks") {
      DELETEPATH("1:/update.ks").
    }
    Log ("Update: " + updatescript) to OrbiterLog.
  ...
  IF ADDONS:RT:HASCONNECTION(SHIP) and inflight {
    IF EXISTS(afp) {
      log ("Inflight - Boot: " + updateScript) to OrbiterLog.
      DOWNLOAD(updateScript).
      Switch to 0.
      MOVEPATH(afp,"oOrbiter-xinflight.update.ks").
      SWITCH TO 1.
      IF EXISTS("1:/update.ks") {
        DELETEPATH("1:/update.ks").
      }
