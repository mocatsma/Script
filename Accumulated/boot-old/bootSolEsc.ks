...
FUNCTION DELAY {
  ...
  Print "Delay Time Function AccTime = " + accTime.
}
// Get a file from KSC
FUNCTION DOWNLOAD {
  PARAMETER name.
  ...
  print "Download(): " + name.
  IF exists(lfp) {
    print "Boot Local File in 1: Deletepath".
    DELETEPATH(lfp).
  }
  IF exists(afp) {
    print "Boot Download: line 53 exists archived: in 0 so COPYPATH: " + name.
    COPYPATH(afp,lfp).//copy from archive
  }
}
// Put a file on KSC Archive
FUNCTION UPLOAD {
  PARAMETER name.
  DELAY().
  local lfp is "1:/" + name. //local file path
  local afp is "0:/" + name. //archive file path
  local old is "0:/oo" + name.//archive original
  IF EXISTS(afp) {
    DELETEPATH(afp).
  }
  IF EXISTS(lfp) {
...
IF ADDONS:RT:HASCONNECTION(SHIP) {
  IF EXISTS(afp) {
    ...
    SWITCH TO 0.
    MOVEPATH(afp,"oKerbinGeo-1.update.ks").
    SWITCH TO 1.
